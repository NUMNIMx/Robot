...
def detect_blue_bottles(image):
    ...
    for contour in contours:
        ...
        if area > 50:  # ขนาดขั้นต่ำของวัตถุ 
            ...
            print(lst_x[-1])
            if lst_x[-1] < 0.6:
                detected_bottles.append((x+2, y-40, w, h*5, area))
            if lst_x[-1] >=0.6 and lst_x[-1] < 0.9:
                detected_bottles.append((x+2, y-60, w, h+100, area))
            if  lst_x[-1] >= 0.9 :
                detected_bottles.append((x+2, y-110, w, h+180, area))
...
if __name__ == '__main__':
    ...
    ep_chassis.move(x=1.2, y=0, z=0, xy_speed=0.7)
