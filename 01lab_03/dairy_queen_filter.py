...
time_plot_values = []
def adr_csv(adr, time_values):
    with open('ad_r.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Time', 'Sharp Right'])
        for t, r in zip(time_values, adr):
            writer.writerow([t, r])
def adl_csv(adl, time_values):
    with open('ad_l.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Time', 'Sharp Right'])
        for t, r in zip(time_values, adl):
            writer.writerow([t, r])
def tof_csv(l_tof, time_values):
    with open('l_tof.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Time', 'Sharp Right'])
        for t, r in zip(time_values, l_tof):
            writer.writerow([t, r])
...
def sub_data_handler(sub_info):
    global previous_filtered_tof
    distance = sub_info
    l_tof.append(int(distance[0]))
    # state(l_tof, ad)
    raw_tof = int(distance[0])
    filtered_tof = low_pass_filter2(raw_tof,previous_filtered_tof,alpha_tof)
    previous_filtered_tof = filtered_tof
    filtered_tof_values.append(filtered_tof)

    state_tof(filtered_tof_values)
...
def low_pass_filter2(current_value, previous_filtered_value, alpha_tof):
    # Apply the IIR filter
    filtered_value2 = current_value + (alpha_tof) * previous_filtered_tof
...
if __name__ == '__main__':
    ...
    prev_err_x = err_x
    time.sleep(0.01)
    adr_csv(ad['right'], time_values)
    adl_csv(ad['left'], time_values)
    tof_csv(l_tof, time_values)
