...
if __name__ == "__main__":
        ...
        if len(markers) != 0:  # target found
            ...
            if 0.7 < score < 0.85:
                if stop_start_time is None:
                    stop_start_time = time.time()  # Start the timer
                elif time.time() - stop_start_time >= 2:
                    ep_blaster.fire()  # Fire the blaster after 2 seconds of target lock
                    stop_start_time = None  # Reset the timer after firing
                else:
                     stop_start_time = None  # Reset the timer if score condition isn't met
            else:
                ep_gimbal.drive_speed(pitch_speed=0, yaw_speed=0)
                stop_start_time = None  # Reset the timer if no target found
            err_x = center_x - x
            err_y = center_y - y
    ...
    y_point4 = [i[0] for i in data_pith_yaw]
    y_point5 = [i[1] for i in data_pith_yaw]
    y_point6 = [i[2] for i in data_pith_yaw]
    y_point7 = [i[3] for i in data_pith_yaw]
    y_point8 = [i[4] for i in data_pith_yaw]
    y_point9 = [i[5] for i in data_pith_yaw]

    plt.plot(y_point8, y_point9)
    plt.plot(y_point8, y_point4)
    plt.legend(["Center X", "X"])
    plt.show()
